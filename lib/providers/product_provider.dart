...
class ProductProvider with ChangeNotifier {
  ...
  ProductModel? findProductById(String productId) {
    if (_products.where((e) => e.productId == productId).isEmpty) {
      return null;
    }
    return _products.firstWhere((e) => e.productId == productId);
  }

  List<ProductModel> findProductsByCategory({required String categoryName}) {
    if (_products
        .where(
          (e) => e.productCategory.toLowerCase() == categoryName.toLowerCase(),
        )
        .isEmpty) {
      return [];
    }
    return _products.where((e) => e.productCategory == categoryName).toList();
  }
